...
{
    /**
     * Get translated value for a model attribute (record-level).
    ...
    /**
     * Default UI string dictionary (fallback if DB row missing).
     */
    ...
    /**
     * Static UI string translations. Loads from DB (cached) then falls back to defaults.
     */
    public static function ui(string $key): string
    {
        $lang = session('lang', 'en');
        $dict = Cache::remember('ui_dict', 600, function () {
            $out = ['en' => [], 'hi' => []];
            try {
                if (Schema::hasTable('ui_translations')) {
                    foreach (\App\Models\UiTranslation::all() as $r) {
                        $out['en'][$r->key] = $r->en_value;
                        $out['hi'][$r->key] = $r->hi_value ?: $r->en_value;
                    }
                }
            } catch (\Throwable $e) { /* schema missing before migrate */ }
            return $out;
        });
        $defaults = self::defaults();
        return $dict[$lang][$key] ?? $defaults[$lang][$key] ?? $defaults['en'][$key] ?? $key;
    }

    public static function flushCache(): void
    {
        Cache::forget('ui_dict');
    }
}
